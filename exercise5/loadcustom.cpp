...
SimpleMeshData load_simple_binary_mesh( char const* aPath )
{
	if( std::FILE* fin = std::fopen( aPath, "rb" ) )
	{
		...
		if( 0 != std::memcmp( magic, kFileMagic, sizeof(kFileMagic) ) )
			throw Error( "'{}': not a COMP3811 mesh\n", aPath );
	...
	throw Error( "load_simple_binary_mesh(): Unable to open '{}' for reading", aPath );
}
		...
		while( aBytes )
		{
			...
			if( 0 == ret )
			{
				if( auto const err = std::ferror( aFile ) )
					throw Error( "fread_(): error while reading {} bytes : {}", aBytes, err );
				if( std::feof( aFile ) )
					throw Error( "fread_(): unexpected EOF ({} still to read)", aBytes );
			}
